...
with sync_playwright() as p:
    ...
    while True:
        ...
        attempt[0] = 0
        _new_term_found = False
        while attempt[0] < 30:
            raw = page.screenshot()
            screenshot = np.frombuffer(raw, np.uint8)
            screen = cv2.imdecode(screenshot, cv2.IMREAD_COLOR)
            template = cv2.imread("src/k_terminal.png")
            result = cv2.matchTemplate(screen, template, cv2.TM_CCOEFF_NORMED)
            _, max_val, _, max_loc = cv2.minMaxLoc(result)
            attempt[0] += 1
            sys.stdout.write(f"\r{SID} {C['yellow']}waiting for k_terminal.png... attempt {attempt[0]} conf={max_val:.3f}{C['reset']}  ")
            sys.stdout.flush()
            if max_val >= 0.6:
                _new_term_found = True
                _new_term_loc = max_loc
                _new_term_tpl_shape = template.shape[:2]
                break
            # save debug screenshot every 5 attempts
            if attempt[0] % 5 == 0:
                dbg_path = f"debug_k_terminal_attempt{attempt[0]}.png"
        ...
        if not _new_term_found:
            log(f"k_terminal.png never matched after {attempt[0]} attempts — saving debug screenshot", "red")
        ...
        try:
            h, w = _new_term_tpl_shape
            click_x = _new_term_loc[0] + w // 2
            click_y = _new_term_loc[1] + h // 2
            log(f"[click 1] Clicking k_terminal at ({click_x}, {click_y})", "grey")
            page.mouse.click(click_x, click_y)
            page.wait_for_timeout(3000)
            log(f"[click 2] Clicking again to focus terminal", "grey")
            page.mouse.click(click_x, click_y)
            page.wait_for_timeout(3000)
        except Exception as e:
